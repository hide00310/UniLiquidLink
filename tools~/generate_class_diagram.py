...
def run_pyreverse(package_name):
    ...
    """
    project_name = package_name.replace(os.sep, "_").replace("/", "_")
    result = subprocess.run(
        [
            "pyreverse", "-o", "puml", "-p", project_name, package_name,
            "--filter-mode", "ALL",
            "-k",
        ],
        cwd=WEBSOCKETLIB,
    ...
    classes_path = os.path.join(WEBSOCKETLIB, f"classes_{project_name}.puml")
    packages_path = os.path.join(WEBSOCKETLIB, f"packages_{project_name}.puml")
...
def render_html(md_content):
    ...
    try:
        pandoc_cmd = (
            ...
            f'-o "{HTML_FILE}" --filter "{filter_path}" '
            # "--verbose"
        )
