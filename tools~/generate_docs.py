...
PROJECT_ROOT = os.path.normpath(os.path.join(SCRIPT_DIR, "Analyzer/LLiquidLink"))
WEBSOCKETLIB = os.path.normpath(os.path.join(SCRIPT_DIR, ".."))

CSPROJ = os.path.join(PROJECT_ROOT, "LLiquidLink.csproj")
BUILD_DIR = os.path.join(PROJECT_ROOT, "Temp", "bin", "Debug")
XML_PATH = os.path.join(BUILD_DIR, "LLiquidLink.xml")
OUT_FILE = os.path.join(WEBSOCKETLIB, "Docs~", "Server.md")
OUT_HTML_FILE = os.path.join(Path(OUT_FILE).parent, Path(OUT_FILE).stem + '.html')
...
def render_html(md_content, html_out_file):
    """Generate ClassDiagram.html via pandoc with plantuml_pandoc_filter.py."""
    try:
        pandoc_cmd = (
            "pandoc --from markdown --to html5 "
            "--metadata 'title=Class Diagrams' "
            f'-o "{html_out_file}"'
            # "--verbose"
        )
        cmd = ["powershell", "-Command", pandoc_cmd]
        print(f"call: {cmd}")
        p1 = subprocess.run(
            cmd,
            input=md_content,
        ...
        if p1.returncode != 0:
            print(f"[WARN] pandoc html conversion failed {p1.stdout} {p1.stderr}", file=sys.stderr)
            return
    except FileNotFoundError as e:
        print(f"[WARN] render_html skipped: {e}", file=sys.stderr)
        return
    print(f"[DONE] HTML generated: {html_out_file}")
