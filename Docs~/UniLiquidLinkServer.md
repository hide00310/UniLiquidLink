...
- `JsonRpc_ResolveChain(RpcResolveChainParam)` — Resolve a chain of property accesses and a terminal method call on a Unity object, dispatching each step server-side. Called via the registered `JsonRpc_ResolveChain` RPC method.
- `JsonRpc_ResolveChainSet(RpcResolveChainSetParam)` — Resolve a chain of property accesses on a Unity object and assign a value to the terminal property. Called via the registered `JsonRpc_ResolveChainSet` RPC method.
