...
[TestFixture]
public class RpcRegistryAllTests
{
    MethodCaller _caller;
    RpcRegistry _rpc;
    JsonSerializerChain _chain;
    JsonSerializerOptions _mainOptions;

    [SetUp]
    public void SetUp()
    {
        _chain = new JsonSerializerChain();
        _mainOptions = _chain.Options[(int)JsonSerializerChain.Stage.Main];
        _rpc = new RpcRegistry(() => new UniLiquidLink.Server.NullLogger());
        _caller = new MethodCaller(() => new UniLiquidLink.Server.NullLogger(), _chain, _rpc.Searcher);
    ...
    {
        _rpc.AddConverterAndRegister(_mainOptions, new FooConverter());
        _rpc.AddRpcAllGetProperty(typeof(Foo));

        var root = JsonDocument.Parse(
            @"{""rpcType"":""" + typeof(FooRpc).FullName + @""",""Value"":21}").RootElement;

        var field = _caller.JsonRpc_ResolveChain(new RpcResolveChainParam { obj = root, steps = new RpcChainStep[0], method = "Value", args = null });
        Assert.AreEqual(21, field);

        var prop = _caller.JsonRpc_ResolveChain(new RpcResolveChainParam { obj = root, steps = new RpcChainStep[0], method = "Doubled", args = null });
    ...
    public void AddRpcAllSetProperty_StaticField_Assigns()
    {
        ...
        _caller.JsonRpc_ResolveChainSet(new RpcResolveChainSetParam { obj = root, steps = new RpcChainStep[0], property = "Counter", value = value });
    ...
    {
        _rpc.AddConverterAndRegister(_mainOptions, new ItemConverter());
        ...
        var prop = _caller.JsonRpc_ResolveChain(new RpcResolveChainParam { obj = root, steps = new RpcChainStep[0], method = "Tripled", args = null });
    ...
    public void ResolveChain_OrgType_ResolvesConcreteSubtypeOverRegisteredBaseType()
    {
        ...
        typeResolver.RegisterAssembly(typeof(RpcRegistryAllTests).Assembly);
        _rpc = new RpcRegistry(() => new UniLiquidLink.Server.NullLogger(), typeResolver);
        _caller = new MethodCaller(() => new UniLiquidLink.Server.NullLogger(), _chain, _rpc.Searcher);
        _rpc.AddConverterAndRegister(_mainOptions, new BaseConverter());
        ...
        var result = _caller.JsonRpc_ResolveChain(new RpcResolveChainParam { obj = root, steps = new RpcChainStep[0], method = "DerivedMethod", args = null });
    ...
    public void SaveRpcNamesCsv_NestedStatic_WritesSimpleClassName()
    {
        ...
        try
        {
            ...
            StringAssert.Contains(
                "RpcRegistryAllTests.Outer.Inner.InnerStatic,Inner,InnerStatic", csv);
        }
    ...
    public void ResolveChain_NullObj_RootProperty_DirectTerminal()
    {
        ...
        var result = _caller.JsonRpc_ResolveChain(
            new RpcResolveChainParam { obj = nullRoot, steps = new RpcChainStep[0], method = "MyFoo", args = null });
    ...
    public void ResolveChain_NullObj_ChainedStep_ResolvesField()
    {
        ...
        var result = _caller.JsonRpc_ResolveChain(
            new RpcResolveChainParam { obj = nullRoot, steps = steps, method = "Value", args = null });
    ...
    public void ResolveChain_NullObj_UnknownRootProperty_Throws()
    {
        ...
        Assert.Throws<ArgumentException>(() =>
            _caller.JsonRpc_ResolveChain(
                new RpcResolveChainParam { obj = nullRoot, steps = new RpcChainStep[0], method = "Nonexistent", args = null }));
    }
