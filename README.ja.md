...
Python の pip パッケージは現時点では配布していないため、以下の手順で手動セットアップしてください。Unity 側は Unity Package Manager (UPM) の git パッケージとしてインストールできます。

### Unity（C#）側

1. Unity で `Window > Package Manager > + > Add package from git URL...` を開き、以下を入力します。
   ```
   https://github.com/hide00310/UniLiquidLink.git#v0.1.0
   ```
   （タグは最新のリリースバージョンに読み替えてください。`UniLiquidLink.asmdef` により Editor 専用アセンブリとして扱われます。）
2. `UniLiquidLink.asmdef` は以下の precompiled DLL を参照します。プロジェクトにこれらが存在しないとコンパイルエラーになります。
...
`Cube Demo` と `All Features Tour` を1つのウィンドウにまとめた `UniLiquidLink Samples` サンプルで動作確認できます。`Window > Package Manager` で UniLiquidLink パッケージを選択し、`Samples` タブの `UniLiquidLink Samples` の `Import` をクリックしてください。`Assets/Samples/UniLiquidLink/<version>/UniLiquidLinkSample/` にコピーされます。

1. Unity メニューから `UniLiquidLink/Samples/Sample Window` を実行し、`Cube Demo` を選んで `Start` を押します。コマンドを入力する必要はありません。Pythonコマンドは既定で `python`、ミドルウェアのスクリプトパスは自動的に解決されます。
C# 側のサーバーを起動します（既定で `http://localhost:8700` で待ち受けます）。

   ```csharp
   // CubeDemoServer.cs (抜粋)
   server = new Server();
