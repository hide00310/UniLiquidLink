...
最小サンプル `Cube Demo` で動作確認できます。`Window > Package Manager` で UniLiquidLink パッケージを選択し、`Samples` タブの `Cube Demo` の `Import` をクリックしてください。`Assets/Samples/UniLiquidLink/<version>/CubeDemo/` にコピーされます。

1. Unity メニューから `UniLiquidLink/Samples/Cube Demo Server Window` を実行し、
Pythonミドルウェアのスタートコマンドを設定します
例 : `python Assets/Samples/UniLiquidLink/<version>/CubeDemo/run_middleware_server.py`
